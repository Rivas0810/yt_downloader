...
let statusTimer;
let isPanelInjected = false; // Bandera para evitar inyecciones duplicadas
...
// ==========================================
// 2. LÓGICA DE INYECCIÓN Y EVENTOS
// ==========================================

function initializePanel() {
    // Si ya está inyectado, salimos.
    if (isPanelInjected || document.getElementById('flask-downloader-panel')) {
        return;
    }
    ...
    // Estructura: Botón Toggle + Contenido
    panel.innerHTML = `
    ...
    dlVideoBtn.addEventListener('click', () => {
        ...
        .then(data => {
            showStatusMessage(data.msg || '¡Solicitud enviada!', 'success');
        })
        .catch(error => {
            console.error(error);
    ...
    submitAudioBtn.addEventListener('click', () => {
        ...
        .then(response => response.json())
        .then(data => {
            showStatusMessage(data.msg || '¡Audio enviado!', 'success'); 
    ...
    console.log('--- Panel de Descarga Inyectado y Eventos Activos ---');
}

function removePanel() {
    const existingPanel = document.getElementById('flask-downloader-panel');
    if (existingPanel) {
        existingPanel.remove();
        isPanelInjected = false;
        console.log('Panel removido al salir de la página de video.');
    }
}

// ==========================================
// 3. DETECCIÓN DE NAVEGACIÓN (SPA)
// ==========================================
...
function checkAndInjectPanel() {
    ...
    if (isVideoPage && !existingPanel) {
        initializePanel();
    } else if (!isVideoPage && existingPanel) {
        removePanel();
    }
    // Casos C y D (ya está bien) no requieren acción.
}

window.addEventListener('yt-navigate-finish', () => {
    setTimeout(checkAndInjectPanel, 100); 
});
...
// Ejecución inicial: Llama a la función al cargar la extensión por primera vez
checkAndInjectPanel();

console.log('--- Panel de Descarga Inicializador Cargado (Modo SPA) ---');